...
if (!userPrompt && !specPath && !resume) {
  ...
  console.error('       bun run claude-harness/index.ts --spec <path-to-spec.md> [options]');
  console.error("       bun run claude-harness/index.ts --resume [options]");
...
if (resume) {
  log("HARNESS", `Resume from ${resolve("workspace/claude")} (progress.json + spec.md)`);
